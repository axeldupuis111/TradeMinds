...
export default function UpgradePage() {
  ...
  async function handleCheckout(plan: "plus" | "premium") {
    ...
    try {
      ...
      const res = await fetch("/api/stripe/checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ plan, interval, locale: lang, ref: readAttributionRef() }),
      });
  ...
  return (
    <div>
      ...
      {showCanceledToast && (
        /* ⚠️ `status` : un paiement abandonné informe, il n'annonce pas un échec. */
        <div role="status" aria-live="polite" className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-surface border border-border text-foreground px-6 py-3 rounded-lg shadow-lg flex items-center gap-3">
