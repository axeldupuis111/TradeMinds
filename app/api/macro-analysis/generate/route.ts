 ...
 *
 * To raise quality at higher cost, switch BRIEF_MODEL to "claude-opus-5" — and
 * add its price to PRICING in lib/ai-cost-log.ts in the same commit, or the
 * logged cost silently becomes zero for an unknown model.
 */
...
/**
 * ⚠️ PASSÉ DE `claude-sonnet-4-6` À `claude-sonnet-5` LE 2026-09-18, et c'est
 * une décision de coût autant que de qualité :
 *
 *   - LE TARIF EST IDENTIQUE (3 $ / 15 $ le million, cf. `lib/ai-cost-log.ts`),
 *     donc la bascule ne change rien au prix par token ;
 *   - le coach premium tourne sur Sonnet 5 depuis le 2026-08-14, et le brief
 *     macro est l'artefact IA le plus VISIBLE du produit : il est lu chaque
 *     matin par tous les abonnés premium. Laisser la génération précédente sur
 *     la seule surface que tout le monde lit n'avait aucune raison d'être ;
 *   - le volume est d'UN appel par jour pour le produit ENTIER, pas par
 *     utilisateur. Même en comptant large, le poste reste sous la dizaine
 *     d'euros par mois et ne dépend pas du nombre d'abonnés.
 *
 * ⚠️ LE COÛT RÉEL N'A JAMAIS ÉTÉ MESURÉ, faute de pouvoir le journaliser : voir
 * `migrations/20260918_product_events_systeme.sql`. La mesure est désormais
 * branchée et s'activera seule le jour où la migration passera.
 */
const BRIEF_MODEL = "claude-sonnet-5";
/** ⚠️ Identifiant épinglé, le même que le coach : un alias peut glisser. */
...
async function createWithWebSearch(
  ...
  deadline: number,
  /**
   * ⚠️ LES COMPTEURS S'ACCUMULENT ICI. Chaque reprise de `pause_turn` est un
   * appel facturé de plus, et l'historique complet lui est renvoyé : ne
   * journaliser que le dernier message sous-estimerait le coût d'un facteur
   * égal au nombre de recherches. C'est exactement l'erreur dont ce dépôt garde
   * la trace sur `compiler-strategie` (majorant faux dans les deux sens).
   */
  compteurs: AiUsage[],
...
Donne 4 à 6 entrées dans "assets" (les classes les plus pertinentes du jour ; "direction" décrit la dynamique attendue de la séance, "volatile" si le sens est incertain mais le mouvement probable ; pour "rates", raisonne sur les rendements obligataires). Donne 3 à 5 thèmes et 3 à 5 éléments à surveiller. Pour "outlook", sois CONCRET et chiffré quand c'est pertinent, en nommant les classes d'actifs et les dynamiques attendues (hausse/baisse de volatilité, pression sur tel actif, etc.). Dans "assets" comme dans "outlook", reste une analyse de CONTEXTE : décris ce à quoi s'attendre, ne dis JAMAIS d'acheter, vendre, ou prendre une position. Reste concis.`;
  ...
  /**
   * ⚠️⚠️ LE SEUL APPEL IA QUOTIDIEN DU PRODUIT N'ÉTAIT MESURÉ NULLE PART. Sur
   * les 43 jours de journalisation (2026-08-06 → 09-17), la base compte 96
   * appels pour 2,06 € au total — et pas UN seul brief macro, alors qu'il
   * tourne tous les matins avec Sonnet, jusqu'à cinq recherches web et trois
   * traductions. C'est très probablement le premier poste de dépense, et c'est
   * le seul invisible.
   *
   * ⚠️ LES COMPTEURS DU BRIEF ET DES TRADUCTIONS SONT JOURNALISÉS SÉPARÉMENT :
   * ils n'emploient pas le même modèle, donc pas le même tarif, et les
   * additionner sous un seul nom de modèle donnerait un coût faux.
   */
  const compteursBrief: AiUsage[] = [];
  try {
    const msg = await createWithWebSearch(
      client,
      briefPrompt,
      Date.now() + BRIEF_BUDGET_MS,
      compteursBrief,
    );
   ...
   * quand même été facturé : ne compter que les succès sous-estimerait le coût
   * exactement les jours où il est le plus élevé.
   */
  if (compteursBrief.length > 0) {
    await logAiCost(supabase, null, {
      route: "macro-brief",
      model: BRIEF_MODEL,
      plan: "systeme",
      usage: sumUsage(compteursBrief),
      rounds: compteursBrief.length,
      extra: { date: analysisDate, abouti: frBrief != null },
    });
  }
  ...
  const compteursTraduction: AiUsage[] = [];
...
${JSON.stringify(fr)}`,
      ...
      compteursTraduction.push(tMsg.usage);
  ...
  /** ⚠️ Modèle différent, donc tarif différent : deuxième événement. */
  if (compteursTraduction.length > 0) {
    await logAiCost(supabase, null, {
      route: "macro-traduction",
      model: TRANSLATE_MODEL,
      plan: "systeme",
      usage: sumUsage(compteursTraduction),
      rounds: compteursTraduction.length,
      extra: { date: analysisDate, langues: compteursTraduction.length },
    });
  }
